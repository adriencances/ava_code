...
sys.path.append("/home/acances/Code/ava/tracks_extraction")
from compute_SORT_tracks import compute_tracks
...
class FrameProcessor:
    def __init__(self, w, h, alpha, phase, frames_dir, shots_dir, tracks_dir, normalized_boxes=False):
        ...
        self.output_dir = "/home/acances/Data/Ava_v2.2/annotated_shots_SORT"
    ...
    def get_raw_frame(self, video_id, t, n):
        # t : timestamp index of the video
        # n : frame index in the timestamp (frame indices start at 1)
        frame_file = "{}/{}/{}/{:05d}/{:06d}.jpg".format(self.frames_dir, self.phase, video_id, t, n)
        assert os.path.isfile(frame_file), frame_file
    ...
    def get_annotated_frame(self, frame, bboxes):
        height, width = frame.shape[:2]

        color = (0, 255, 0)
        thickness = 2
        for box, track_id in bboxes:
    ...
    def get_tracks(self, video_id, shot_id):
        tracks_file = "{}/{}/{}/{:05d}_tracks.pkl".format(self.tracks_dir, self.phase, video_id, shot_id)
        with open(tracks_file, "rb") as f:
            tracks = pickle.load(f)
        return tracks
    ...
    def get_bboxes_by_frame(self, tracks, end_frame):
        bboxes_by_frame = [[] for i in range(end_frame)]
        for track_id, track in enumerate(tracks):
            for bbox in track:
                i = int(bbox[0])
                box = bbox[1:5]
                bboxes_by_frame[i].append((box, track_id))
        return bboxes_by_frame
    
    def print_annotated_shot(self, video_id, shot_id):
        N = self.nb_frames_by_timestamp_dict[video_id]
        t1, t2 = self.shot_boundary_timestamps_dict[video_id][shot_id]
        ...
        tracks = self.get_tracks(video_id, shot_id)
        bboxes_by_frame = self.get_bboxes_by_frame(tracks, end_frame)

        output_subdir = "{}/{}/shot_{:05d}".format(self.output_dir, video_id, shot_id)
        Path(output_subdir).mkdir(parents=True, exist_ok=True)
        annotated_frames = []
        for i in range(begin_frame, end_frame):
            t = t1 + i//N
            n = i%N + 1
            raw_frame = self.get_raw_frame(video_id, t, n)
            annotated_frame = self.get_annotated_frame(raw_frame, bboxes_by_frame[i])
            frame_file = "{}/{:04d}.jpg".format(output_subdir, i)
...
if __name__ == "__main__":
    ...
    frame_processor = FrameProcessor(w, h, alpha, phase, frames_dir, shots_dir, tracks_dir)

    video_id = sys.argv[1]
    shot_id = int(sys.argv[2])
    compute_tracks(video_id, shot_id)
    frame_processor.print_annotated_shot(video_id, shot_id)
